...
int _get_file_content(const char* filename, u8** content, long* len) {
	FILE* fp = fopen(filename, "rb");
	if (fp) {
		fseek(fp, 0, SEEK_END);
		*len = ftell(fp);
		*content = (u8*) malloc(*len);
		if (content) {
...
void _free_file_content(u8* content) {
	if (content) free(content);
}

struct s_bctx*
	_create_bctx(u8* content, u32 len) {
	struct s_bctx* b = (s_bctx*) malloc(sizeof(*b));
	if (b) {
		memset(b, 0, sizeof(*b));
		b->r_data = content;
		b->len = len;
...
void _destroy_bctx(struct s_bctx* b) {
	if (b) {
		free(b);
	}
}

void _dump_buf(const unsigned char* buf, int stride) {
	int i, base = 0;
	for (i = 0; i < 64; i++) {
		printf("%02x ", buf[base++]);
		if (i && (i + 1) % 8 == 0) {
...
void _skip_bytes(struct s_bctx* b, int n) {
	b->r_ptr += n;
}

u8 _next_byte(struct s_bctx* b) {
	return b->r_data[b->r_ptr++];
}

u16 _next_word(struct s_bctx* b) {
	u8 b1 = _next_byte(b);
	return b1 << 8 | _next_byte(b);
}

u8* _get_ptr(struct s_bctx* b) {
	return &b->r_data[b->r_ptr];
}

u32 _get_offset(struct s_bctx* b) {
	return b->r_ptr;
}

int _is_eof(struct s_bctx* b) {
	return (b->r_eof || b->r_ptr >= b->len);
}

void _set_eof(struct s_bctx* b) {
	b->r_eof = 1;
}

u32 _bits_try(struct s_bctx* b, int n) {
	u8 buf;
	u32 data = 0;
	assert(n <= 32);
	while (b->rb_bits < n) {
...
void _bits_skip(struct s_bctx* b, int n) {
	if (b->rb_bits >= n) {
		b->rb_bits -= n;
...
u32 _bits_read(struct s_bctx* b, int n) {
	u32 data = _bits_try(b, n);
	_bits_skip(b, n);
	return data;
}

void _bits_clear(struct s_bctx* b) {
	b->rb_buf = 0;
	b->rb_bits = 0;
}
...
struct s_ht_ary 
	...
	if (count > 0) {
		a.count = count;
		a.v = (s_ht_vlc*) malloc(sizeof(*a.v) * count); /* free in destroy_jctx */
		memset(a.v, 0, sizeof(*a.v) * count);
	}
	return a;
}

void _destroy_ht_ary(struct s_ht_tbl* ht) {
	int i;
...
struct s_jctx* _create_jctx(void) {
	struct s_jctx* j = (s_jctx*)malloc(sizeof(*j));
	if (j) {
		memset(j, 0, sizeof(*j));
	}
	return j;
...
void _save_to_ppm(struct s_jctx* j) {
	if (j->scan_out) {
...
void _get_qt_table(struct s_bctx* b, struct s_jctx* j) {
...
const char* _print_binary(int x, int len) {
	static char b[33] = { 0 };
...
void _print_ht_table(struct s_jctx* j, int idx) {
	int i, n;
...
void _get_ht_table(struct s_bctx* b, struct s_jctx* j) {
	int i, w, ht_base;
	u16 len = _next_word(b);
...
void _decode_frame(struct s_bctx* b, struct s_jctx* j) {
	int i, hmax = 0, vmax = 0;
	u16 len = _next_word(b);
	u8 P = _next_byte(b);
...
void _idct_col(const s32* blk, u8* out, int stride) {
	s32 x0, x1, x2, x3, x4, x5, x6, x7, x8;
	if (!((x1 = blk[8 * 4] << 8)
		| (x2 = blk[8 * 6])
		| (x3 = blk[8 * 2])
...
void _h1v1_convert_mcu(struct s_jctx* j, int mcu_n) {
	int x, y, obase, pbase;
	u8* out = j->scan_out;
...
void _grayscale_convert_mcu(struct s_jctx* j, int mcu_n) {
	int y, obase, pbase;
	u8* out = j->scan_out;
	u8* py = j->comp[0].pixels;
	pbase = 0;
...
int _check_vlc_in_ht(struct s_bctx* b, struct s_ht_tbl* ht, u8* code) {
	int i, n, val;
...
static void _decode_block(struct s_bctx* b, struct s_jctx* j, int comp_id) {
	int i, ai, val;
...
void _decode_scan(struct s_bctx* b, struct s_jctx* j) {
	int i;
	u16 len = _next_word(b);
...
void _decode_dri(struct s_bctx* b, struct s_jctx* j) {
	u16 lr = _next_word(b);
	u16 ri = _next_word(b);
...
int _skip_segment(struct s_bctx* b) {
	u16 len = _next_word(b);
	_skip_bytes(b, len - 2);
	return len;
}
...
int _decode(struct s_bctx* b, struct s_jctx* j) {
	while (!_is_eof(b)) {
